...
# endpoint use the much tighter `PROBE_TIMEOUT_SECONDS` instead, and
...
def _run(
    ...
    *,
    ...
    than a `TimeoutExpired` traveling up into a handler. Pass `timeout=None`
    only for a call that blocks by contract and carries its own bound.
    """
    try:
...
def status(namespace: str = DEFAULT_NAMESPACE, component: str = "api") -> dict[str, Any]:
    ...
    The per-track health reads only happen in Kubernetes mode (#3858): in any
    other mode the tracks report the mode message rather than the result of a
    kubectl call that had no cluster to reach.
    """
    mode = current_mode()
...
def _finalize_promotion(
    ...
    message = (
        ...
        f"and stable back to its resting {_replica_count(resting)}"
    )
...
def promote(
    ...
    *,
    ...
    if new_weight >= 100:
        return _finalize_promotion(state, canary_health, namespace, resting, spec, component)
    ...
    message = f"Promoted canary to {plan.describe()}"
    ops_module.record_canary_action("promote", "success", message, component=component)
    logger.info(
        "canary: %s",
        message,
