...
- **Wrapped observability logins** (`nyxgpt ops credentials`) — prints the Grafana and GlitchTip admin credentials from whichever source provisioned them, and `nyxgpt cloud credentials` reads them off a deployment over the wrapped SSH access path, so signing into either UI never means SSHing to a box and reading a secret file by hand; the values stay CLI-only and are never returned by the API — see [docs/ops.md](docs/ops.md#nyxgpt-ops-credentials)
...
- **PyPI builds for acceptance testing** (`nyxgpt release publish`) — one pipeline publishes the release-branch tip as an on-demand `rc` or as the release itself (`stable`, via the owner's ceremony), so a clean machine or EC2 instance can acceptance-test unreleased code by exact pin with no checkout; nothing is published on a schedule, and `pip install nyxgpt` still resolves to the latest stable release — see [docs/cloud.md](docs/cloud.md#pypi-publishing-rc-and-stable)
