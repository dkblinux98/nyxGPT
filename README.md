...
- **Error tracking** (self-hosted GlitchTip) — local-only backend exception and web UI client error reporting via the Sentry SDK protocol, auto-started with `nyxgpt ops install` (creating a project and DSN is the one step that still needs a human), linked from the SRE/admin dashboard (`/admin`)
