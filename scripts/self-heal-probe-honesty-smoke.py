...
* **group-denied** -- the owner's *re-test* condition, and the reason #3812 was
  ...
  --clear-groups``. A bare ``docker`` call there is denied; the shipped probe
  must nonetheless report the running container, having retried through ``sg
  docker``. The denial itself comes from a stub ``docker`` first on PATH that
  keys off nothing but the caller's group set, so this half does not depend on
  whether the host's socket happens to be group-protected -- on GitHub's
...
# The stub `docker` reports this service as the one running container, standing
...
CLI_DENIED_STDERR = (
    ...
    '"http://%2Fvar%2Frun%2Fdocker.sock/v1.47/containers/json?all=1": dial unix '
    "/var/run/docker.sock: connect: permission denied"
)
COMPOSE_DENIED_STDERR = (
...
# A stand-in `docker` whose only input is the caller's group set: denied
# without the `docker` group, working with it. See `_write_stub_docker`.
STUB_DOCKER_SOURCE = '''#!/usr/bin/env python3
"""A `docker` that is denied to a session without the `docker` group.

Generated by scripts/self-heal-probe-honesty-smoke.py -- not shipped.
"""
import json
import os
import sys
...
if is_compose and "ps" in argv:
    ...
    sys.exit(0)
if argv[:1] == ["info"]:
    sys.stdout.write("00.0.0-nyxgpt-stub\\n")
    sys.exit(0)
# Everything else (`docker ps --filter ...`, `docker inspect ...`) answers
# emptily and successfully: "Docker replied, and there is no such container",
# which is the honest `absent` the panel should show for one.
sys.exit(0)
'''
...
def _write_stub_docker(bin_dir: Path, docker_gid: int) -> Path:
    """Write a `docker` whose success depends only on holding `docker_gid`.

    **Why a stub and not the host's real socket.** The condition under test is
    a *session* that lacks the `docker` group, and the property under test is
    that `_docker_run` retries such a failure through `sg docker`. Asserting
    that against the real socket makes the check depend on the host's socket
    permissions: on a GitHub `ubuntu-latest` runner the socket is not
    group-protected, so dropping the group denies nothing and the assertion
    has nothing to bite on -- the script correctly refused to pass vacuously
    and failed instead. This stub supplies the denial itself, from the one
    input that genuinely differs between the two sessions -- the group set --
    so the retry is exercised identically on any host, with no daemon and no
    socket permissions involved. What stays real is the part that carries the
    property: the group set is genuinely stripped with
    `setpriv --clear-groups`, and `sg docker` genuinely restores it out of
    `/etc/group`.

    The gid is baked into the file rather than read from the environment: the
    child crosses `sudo`, `setpriv` and `sg`, and a variable lost at any of
    those hops would silently turn "denied here" into "denied everywhere",
    which fails as a false alarm instead of proving anything.
    """
    bin_dir.mkdir(parents=True, exist_ok=True)
    stub = bin_dir / "docker"
    stub.write_text(
        STUB_DOCKER_SOURCE.format(
            gid=docker_gid,
...
def run_group_denied_child() -> None:
    ...
    the only place the difference shows: the parent process holds the group
    and would prove nothing.
    """
    stub = os.environ.get("NYXGPT_PROBE_STUB_DOCKER", "")
    if not stub:
        die("the child was started without NYXGPT_PROBE_STUB_DOCKER; it cannot check its own setup")
    ...
    if resolved != stub:
        die(
            f"the stub docker is not what this session resolves: `docker` is {resolved!r}, "
            f"expected {stub!r} -- PATH did not survive into the child, so this half would "
            "be measuring the host's real socket instead of the injected denial"
        )
    ...
    if bare.returncode == 0:
        die(
            "a bare docker call succeeded without the group -- the injection has no "
            "teeth, so a regression of the retry would pass here silently"
        )
    ...
    # The pre-condition for the retry itself: `sg docker` must be able to run
    # *this* docker from here. If the hop cannot run, or runs a different
    # binary, the assertions below would be measuring something else -- so say
    # so, rather than reporting a probe failure as a regression of the fix.
    hop_which = subprocess.run(
        ["sg", "docker", "-c", "command -v docker"], capture_output=True, text=True
    )
    if hop_which.returncode != 0 or (hop_which.stdout or "").strip() != stub:
        die(
            "`sg docker` does not run the stub from this session "
            f"(rc={hop_which.returncode}, docker={(hop_which.stdout or '').strip()!r}, "
            f"stderr={hop_which.stderr!r}) -- the hop under test cannot be exercised here"
        )
    hop_info = subprocess.run(
        ["sg", "docker", "-c", "docker info --format '{{.ServerVersion}}'"],
        capture_output=True,
        text=True,
    )
    if hop_info.returncode != 0:
        die(
            f"`sg docker` cannot reach docker either (rc={hop_info.returncode}, "
            f"stderr={hop_info.stderr!r}) -- nothing here could distinguish a working "
            "retry from a broken one"
        )
    log(
        "group-denied-child: the same call through `sg docker` succeeds -- "
        "the group set is the only difference between them"
    )
...
def run_group_denied_half(root: Path) -> None:
    ...
    # `sg docker` only applies a membership /etc/group actually records -- for
    # anyone else it asks for the group password, which under a CI runner's
    # closed stdin is a failure, not a prompt. Asked here rather than inferred
    # from `gr_mem`, because what matters is that the hop *works* for this
    # user; stdin is closed and the call bounded so a host that would have
    # prompted fails fast instead of hanging the job.
    ...
    if usable.returncode != 0:
        die(
            f"`sg docker` does not work for this user (rc={usable.returncode}, "
            f"stderr={usable.stderr!r}) -- the membership the hop under test relies on "
            "is not one /etc/group grants here, so nothing below would be evidence"
        )

    stub = _write_stub_docker(root / "stub-bin", docker_gid)
    path_value = f"{stub.parent}:{os.environ.get('PATH', '')}"
    resolved = shutil.which("docker", path=path_value)
    if resolved != str(stub):
        die(f"the stub is not first on the child's PATH: `docker` resolves to {resolved!r}")
    # The stub has to *discriminate*, not merely fail. Proven here, where the
    # group IS held: same binary, same argv, the opposite outcome to the child's.
    held = subprocess.run(
    ...
    if held.returncode != 0:
        die(
            "the stub docker failed even with the docker group held "
            f"(rc={held.returncode}, stderr={held.stderr!r}) -- it would deny every "
            "session equally and so prove nothing about the group"
        )
    log("group-denied: the stub docker answers when the docker group is held")
    ...
    cmd = [
        ...
        f"NYXGPT_PROBE_STUB_DOCKER={stub}",
        "setpriv",
        "--clear-groups",
        f"--reuid={os.getuid()}",
        f"--regid={os.getgid()}",
