...
tell application "Terminal"
    do script "cd '$REPO_DIR' && watch -n 5 'echo \"=== SCRUMMASTER AGENT ===\"; gh run list --workflow=\"Scrummaster Agent - Select and Start Next Issue\" --status in_progress --status queued'"
...
osascript <<EOF
tell application "Terminal"
    do script "cd '$REPO_DIR' && watch -n 5 'echo \"=== DEVELOPER AGENT ===\"; gh run list --workflow=\"Developer Agent Auto-Implement\" --status in_progress --status queued'"
...
osascript <<EOF
tell application "Terminal"
    do script "cd '$REPO_DIR' && watch -n 5 'echo \"=== REVIEWER AGENT ===\"; gh run list --workflow=\"Review Agent Auto-Review\" --status in_progress --status queued'"
