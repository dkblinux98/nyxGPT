...
### branch hygiene
- `reconcile_dead_branches.sh [--delete] [base_branch]` — on-demand **report** of `claude/*`, `feat/*`, `fix/*`, `chore/*` branches, for the human question "what is still out there?". Report-only by default since #3862 (`--delete` to act; `--dry-run` still accepted and is now the default). A branch is deletable only if its **content** is provably on `base_branch` — an ancestor, or every path it touches already identical there (`lib/branch_content.py`) — or it is the head of a PR closed without merging. Age, "no PR exists", a closed linked issue and `git branch --merged` are **not** signals and were each disproven against a real branch set on 2026-08-18. Nothing schedules this and nothing should: branch cleanup is event-driven (ledger D-013), owned by the agent that created the branch. `developer_create_branch.sh` and `developer_ensure_pr_exists.sh` apply the same content check at the moments work changes state.
