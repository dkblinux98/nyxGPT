...
PHASE_ERA = {'Phase 0': 'v1.0.0', 'Phase 1': 'v1.0.0', 'Phase 2': 'v1.0.0',
             'Phase 3': 'v1.0.0', 'Phase 3.5': 'v1.0.0',
             'Phase 4': 'v2.0.0', 'Phase 5': 'v2.0.0', 'Phase 5.5': 'v2.0.0'}
...
def takeaways(issues, dashboard, weeks, open_af):
    ...
    if len(data_weeks) >= 2:
        ...
        out.append({'k': 'Failure trend',
                    'v': f"{cur} acceptance failure{'s' if cur != 1 else ''} in {name} vs {prev} last sprint ({'+' if delta >= 0 else ''}{delta})"})
...
def build_qdata(issues, project_fields):
    ...
    for b in buckets:
        ...
        prev_era = era
        weeks.append({'w': b['w'], 'sname': b.get('label'), 'era': era,
                      'end': b.get('end'), **agg})
...
def main():
    ...
    html = re.sub(r'across all \d+ issues',
                  f"across all {qdata['qtotals']['issues']} issues", html)
