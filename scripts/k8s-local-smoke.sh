...
step "1/6 Bring the stack up: nyxgpt ops install --kubernetes --local"
nyxgpt ops install --kubernetes --local --api-key "$API_KEY" --skip-observability
ok "install --kubernetes --local completed"

step "2/6 The data/LLM tier exists and is Ready"
...
step "4/6 A real chat round-trip"
...
step "5/6 Sessions are shared by every api replica (Cassandra-backed)"
...
step "6/6 Fault injection: the pre-#3786 topology must FAIL this same check"
stop_port_forward
