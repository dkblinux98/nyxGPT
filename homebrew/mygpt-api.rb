class MygptApi < Formula
  ...
  def install
    (bin/"mygpt-api").write <<~EOS
      #!/bin/bash
      set -euo pipefail

      CONFIG_FILE="$HOME/.myGPT/config.ini"
...
cfg.read(os.path.expanduser('~/.myGPT/config.ini'), encoding='utf-8')
...
PY
      ...
      if [ -z "$REPO_DIR" ] || [ -z "$VENV_PY" ]; then
        echo "ERROR: repo_dir or venv_python not set in ~/.myGPT/config.ini" >&2
        exit 1
      fi
      ...
      echo "mygpt-api wrapper starting" >&2
      echo "  repo_dir: $REPO_DIR" >&2
      echo "  venv_python: $VENV_PY" >&2
      echo "  host: $HOST" >&2
      echo "  port: $PORT" >&2

      trap 'echo "mygpt-api wrapper received SIGTERM" >&2' TERM
      trap 'echo "mygpt-api wrapper received SIGINT" >&2' INT

      "$VENV_PY" -m uvicorn mygpt.app:app --host "$HOST" --port "$PORT"
      RC=$?
      echo "mygpt-api uvicorn exited with code $RC" >&2
      exit $RC
    EOS
    system "chmod", "0755", bin/"mygpt-api"
  end

  service do
    run ["/bin/bash", opt_bin/"mygpt-api"]
    keep_alive true
    log_path var/"log/mygpt-api.log"
    error_log_path var/"log/mygpt-api.err.log"
  end
end
