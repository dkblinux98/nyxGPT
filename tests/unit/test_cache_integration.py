...
from nyxgpt.rag.embeddings import embed_texts, embed_text, clear_embedding_cache, _embedding_cache
from nyxgpt.chat import chat, clear_response_cache, _response_cache
...
"""
        ...
        with patch("nyxgpt.rag.embeddings._post_json", return_value=mock_response):
            with patch("nyxgpt.rag.embeddings.load_config") as mock_load:
...
"""
        ...
        with patch("nyxgpt.rag.embeddings._post_json", side_effect=mock_post_json):
            with patch("nyxgpt.rag.embeddings.load_config") as mock_load:
                from configparser import ConfigParser
...
""".format(tmpdir=tmp_path)
        ...
        with patch("nyxgpt.chat.ollama_chat", side_effect=mock_ollama_chat):
            with patch("nyxgpt.chat.load_config") as mock_load:
                from configparser import ConfigParser
                ...
                with patch("nyxgpt.chat.load_session"):
                    with patch("nyxgpt.chat.save_session"):
                        # First call
        ...
        with patch("nyxgpt.chat.ollama_chat", side_effect=mock_ollama_chat):
            with patch("nyxgpt.chat.load_config") as mock_load:
                ...
                with patch("nyxgpt.chat.load_session"):
                    with patch("nyxgpt.chat.save_session"):
                        # First prompt
                        result1 = chat("prompt1", config_path=str(config_file), new=True)
                        assert call_count == 1

                        # Different prompt
                        result2 = chat("prompt2", config_path=str(config_file), new=True)
                        assert call_count == 2  # New API call

                        # Repeat first prompt - should use cache
                        result3 = chat("prompt1", config_path=str(config_file), new=True)
