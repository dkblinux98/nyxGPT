/**
 * Tests for the /api/v1/deploy/switch Next.js proxy route (issue #3245).
 */
...
describe('/api/v1/deploy/switch proxy route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  ...
  it('forwards POST body to backend using default URL', async () => {
    ...
    const { POST } = await import('../../../../../../src/app/api/v1/deploy/switch/route');
    const req = new Request('http://localhost/api/v1/deploy/switch', {
      method: 'POST',
    ...
    expect(calledUrl).toBe('http://127.0.0.1:8000/api/v1/deploy/switch');
    expect(calledOptions.method).toBe('POST');
    expect(JSON.parse(calledOptions.body)).toEqual({ color: 'green' });

    const body = await response.json();
    expect(body).toEqual({ activeColor: 'green' });
  });
  ...
  it('uses NYXGPT_API_BASE_URL when set', async () => {
    ...
    expect(calledUrl).toBe('http://custom-backend:9000/api/v1/deploy/switch');
  });
  ...
  it('passes through backend response status', async () => {
    mockFetch({ ok: false, status: 409, data: { error: 'switch already in progress' } });
  ...
  it('returns 502 with structured error when backend is unreachable', async () => {
    ...
    const body = await response.json();
    expect(body).toEqual({ error: 'Failed to switch deployment color' });
  });
