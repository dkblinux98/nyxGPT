/**
 * Tests for the /api/v1/infra/restart-required Next.js proxy route (#3407).
 *
 * web/src/app/api/v1/infra/restart-required/route.ts forwards POST to the
 * backend /api/v1/infra/restart-required endpoint via apiFetch, passing
...
describe('/api/v1/infra/restart-required POST route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  ...
  it('forwards POST body and method to backend', async () => {
    ...
    const { POST } = await import(
      '../../../../../../src/app/api/v1/infra/restart-required/route'
    );
    const req = new Request('http://localhost/api/v1/infra/restart-required', {
      method: 'POST',
    ...
    expect(calledUrl).toBe('http://127.0.0.1:8000/api/v1/infra/restart-required');
  ...
  it('uses NYXGPT_API_BASE_URL when set', async () => {
    ...
    expect(calledUrl).toBe('http://custom-backend:9000/api/v1/infra/restart-required');
  });
  ...
  it('returns 502 with structured error when backend is unreachable', async () => {
    ...
    expect(body).toEqual({ error: 'infra backend unavailable' });
  });
});
