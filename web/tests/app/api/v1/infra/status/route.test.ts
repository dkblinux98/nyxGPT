/**
 * Tests for the /api/v1/infra/status Next.js proxy route (issue #3344).
 */
...
describe('/api/v1/infra/status proxy route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  ...
  it('forwards GET request to backend using default URL and returns 200', async () => {
    mockFetch({ ok: true, status: 200, data: { terraform: { applied: false }, kubernetes: { deployed: false } } });

    const { GET } = await import('../../../../../../src/app/api/v1/infra/status/route');
    const response = (await GET()) as Response;

    expect(global.fetch).toHaveBeenCalledTimes(1);
    const [calledUrl, calledOptions] = (global.fetch as ReturnType<typeof vi.fn>).mock.calls[0];
    expect(calledUrl).toBe('http://127.0.0.1:8000/api/v1/infra/status');
    expect(calledOptions.method).toBe('GET');

    expect(response.status).toBe(200);
    const body = await response.json();
    expect(body).toEqual({ terraform: { applied: false }, kubernetes: { deployed: false } });
  });
  ...
  it('uses NYXGPT_API_BASE_URL when set', async () => {
    ...
    expect(calledUrl).toBe('http://custom-backend:9000/api/v1/infra/status');
  });
  ...
  it('returns 502 with structured error when backend is unreachable', async () => {
    ...
    const body = await response.json();
    expect(body).toEqual({ error: 'Failed to fetch infra status from backend' });
  });
