/**
 * Tests for the /api/v1/infra/kubernetes/install Next.js proxy route (issue #3344).
 */
...
describe('/api/v1/infra/kubernetes/install proxy route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  ...
  it('forwards POST body to backend using default URL', async () => {
    ...
    const { POST } = await import('../../../../../../../src/app/api/v1/infra/kubernetes/install/route');
    const req = new Request('http://localhost/api/v1/infra/kubernetes/install', {
      method: 'POST',
    ...
    expect(calledUrl).toBe('http://127.0.0.1:8000/api/v1/infra/kubernetes/install');
    expect(calledOptions.method).toBe('POST');
    expect(JSON.parse(calledOptions.body)).toEqual({ api_key: 'secret' });

    const body = await response.json();
    expect(body).toEqual({ deployed: true });
  });
  ...
  it('uses NYXGPT_API_BASE_URL when set', async () => {
    ...
    expect(calledUrl).toBe('http://custom-backend:9000/api/v1/infra/kubernetes/install');
  });
  ...
  it('returns 502 with structured error when backend is unreachable', async () => {
    ...
    const body = await response.json();
    expect(body).toEqual({ error: 'Failed to deploy to Kubernetes' });
  });
