/**
 * Tests for the /api/v1/canary/promote Next.js proxy route (issue #3245).
 */
...
describe('/api/v1/canary/promote proxy route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  ...
  it('forwards POST body to backend using default URL', async () => {
    mockFetch({ ok: true, status: 200, data: { promoted: true } });

    const { POST } = await import('../../../../../../src/app/api/v1/canary/promote/route');
    const req = new Request('http://localhost/api/v1/canary/promote', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ rolloutId: 'abc123' }),
    });
    const response = (await POST(req)) as Response;

    expect(global.fetch).toHaveBeenCalledTimes(1);
    const [calledUrl, calledOptions] = (global.fetch as ReturnType<typeof vi.fn>).mock.calls[0];
    expect(calledUrl).toBe('http://127.0.0.1:8000/api/v1/canary/promote');
    expect(calledOptions.method).toBe('POST');
    expect(JSON.parse(calledOptions.body)).toEqual({ rolloutId: 'abc123' });
  ...
  it('uses NYXGPT_API_BASE_URL when set', async () => {
    ...
    const req = new Request('http://localhost/api/v1/canary/promote', {
      method: 'POST',
      body: JSON.stringify({ rolloutId: 'xyz' }),
    });
    await POST(req);

    const calledUrl: string = (global.fetch as ReturnType<typeof vi.fn>).mock.calls[0][0];
    expect(calledUrl).toBe('http://custom-backend:9000/api/v1/canary/promote');
  });
  ...
  it('returns 502 with structured error when backend is unreachable', async () => {
    ...
    const body = await response.json();
    expect(body).toEqual({ error: 'Failed to promote canary rollout' });
  });
