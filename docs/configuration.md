...
| `chat_timeout_seconds` | Timeout for a single chat request (default: `300`) |
| `think` | Whether the chat model may emit chain-of-thought before answering (default: `false`). The shipped `default_model` is a reasoning model and nyxGPT reads only the answer, so leaving this on makes every message pay for hidden reasoning tokens that are then discarded -- measured at 2 tokens / 0.3s off versus 11,553 tokens / 185s on for the same one-word answer. Turn it on to trade that latency back for the model's reasoning. |
...
**Note:** `base_url` may carry credentials (`http://user:pass@host:11434`) for <!-- pragma: allowlist secret -->
an Ollama behind basic auth. When startup validation rejects the value it
prints the URL back with the `user:pass@` part replaced by `***@`, so the
password does not reach stderr or the logs.
