...
Log in with username `admin` and the password printed by:

```bash
nyxgpt ops credentials --service grafana
```

That reads it from whichever source applies — `[monitoring]
grafana_admin_password` in `~/.nyxGPT/config.ini` (auto-generated by `nyxgpt
wizard`) when set, otherwise the ops-managed secret `nyxgpt ops install`
generates at `~/.nyxGPT/secrets/grafana-admin-password` — and names which one
answered, so there is no need to open either file by hand (#3718); see
[ops.md](ops.md#nyxgpt-ops-credentials). `nyxgpt ops install` resets the
...
chmod 600) — print it with `nyxgpt ops credentials --service glitchtip`
instead of opening that file (#3718); the DSN itself is also written into
