...
| [`k8s-capacity-smoke.yml`](../.github/workflows/k8s-capacity-smoke.yml) | `ubuntu-latest` | That the stack's memory **requests** fit that same node -- the arithmetic no manifest review catches. Applies the pre-#3825 sizing and requires Pods to be left unschedulable, requires the install's capacity preflight to refuse it, then requires the shipped sizing to schedule every Pod (prometheus included) and both canary Deployments to scale up and land. No image builds: scheduling is decided from the Pod spec before any pull | PR/push (paths: `k8s/**`, `src/nyxgpt/ops.py`), dispatch |
