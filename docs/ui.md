...
- **Infrastructure Status** (`/admin/infrastructure`) — status-only reporting of the detected deployment mode, per-mode component state, and which instance serves traffic (#3410). Its Native card also carries the **install mode**: an `ARTIFACT INSTALL` / `DEV INSTALL` badge, and, in dev mode, the checkout whose working tree `api`/`web` are running plus the reminder that this stack is not exercising the artifact path (see [`--dev`](ops.md#--dev-run-the-current-checkout-without-an-artifact-build)). Its Kubernetes card carries the equivalent for that deployment (#3834): what the two images in the cluster were built from — the published artifacts, a checkout's working tree (`--dev`), or `unrecorded` when no marker exists for it on this machine, which is never presented as the artifact default. Like the rest of the page it reports only — switching modes is `nyxgpt up` / `nyxgpt up --dev` (`nyxgpt ops install --kubernetes --local [--dev]` for the cluster). The local Terraform card is labelled **Terraform (local containers)**, because it detects `nyxgpt-tf-*` containers on this machine and an AWS instance Terraform provisioned is a different thing, reported in the AWS section below (#3804).
