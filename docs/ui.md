...
- **AWS Cloud Infrastructure** (`/admin/cloud-infrastructure`) — plan, apply, and destroy the AWS substrate a cloud deployment runs on (VPC, public subnet, an SSH-only security group scoped to your own IP, one EC2 instance), the dashboard counterpart of `nyxgpt cloud infra` (see [Cloud (AWS)](cloud.md#nyxgpt-cloud-infra--provisioning-the-aws-substrate-p6-8-3509)). Shows the recorded substrate and its single open port; teardown requires typing `DESTROY`
