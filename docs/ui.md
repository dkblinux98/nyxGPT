...
- **Infrastructure Status** (`/admin/infrastructure`) — status-only reporting of the detected deployment mode, per-mode component state, and which instance serves traffic (#3410). Its Native card also carries the **install mode**: an `ARTIFACT INSTALL` / `DEV INSTALL` badge, and, in dev mode, the checkout whose working tree `api`/`web` are running plus the reminder that this stack is not exercising the artifact path (see [`--dev`](ops.md#--dev-run-the-current-checkout-without-an-artifact-build)). Like the rest of the page it reports only — switching modes is `nyxgpt up` / `nyxgpt up --dev`. The local Terraform card is labelled **Terraform (local containers)**, because it detects `nyxgpt-tf-*` containers on this machine and an AWS instance Terraform provisioned is a different thing, reported in the AWS section below (#3804).
  - **AWS** — the same page's information-only section for the cloud: the substrate (region, instance, type, public IP, VPC, subnet, security group, key pair, open ports), the release deployed onto it, the access tunnel and a health answer, the Terraform state backend, the deploy/teardown history, and the wrapped `nyxgpt cloud …` commands rendered as text pointers from the backend's own `LIFECYCLE_COMMANDS`. **Nothing in it acts.** Which source answered depends on where the dashboard is running, and the page says so: instance metadata (IMDSv2) when it is served *from* the EC2 instance, Terraform state on the workstation that provisioned it, and **unknown** — never a blank "not provisioned" — on a machine that is neither (#3804). See [Cloud (AWS)](cloud.md#from-the-dashboard-information-only-3804).
