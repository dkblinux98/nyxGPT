...
- `rag_enabled` - whether RAG is enabled globally (`get_rag_enabled` / `[rag] enable_chat_context`). The query cache is only exercised by RAG retrievals, so `enabled=true` with `rag_enabled=false` typically means zero hits/misses -- not a misconfiguration. RAG can still be on for individual chats even when the global default is off, in which case hits/misses may still be non-zero.
...
- **Upload a document** ("Upload Document" button on each collection's card) to ingest a file directly into that collection: pick the file, and it's parsed, chunked, and embedded via the same pipeline as `POST /api/v1/rag/upload` (see [Document Upload](#document-upload)). Success or failure is reported with the resulting `doc_id`, chunk count, and target collection, and the card's document/chunk counts refresh immediately. This is the intended way to add documents to a non-default collection (Owner design decision, 2026-08-01) -- the chat interface does not ingest documents; its paperclip attaches a file to a single message only (see [UI -- File Attachments](ui.md#file-attachments)).
...
**Cleanup note (documents ingested from the pre-#3463-fix chat "Upload file" control):** between PR #3519 merging (2026-07-31) and the owner design decision that removed it (2026-08-01), the chat interface had a "+" upload control that ingested files into RAG, targeting whichever collection was selected in the chat's Collection filter (`"default"` if none was selected) -- indistinguishable from any other document in that collection today. To find them, call `GET /api/v1/rag/documents?collection=<name>` (or check a collection's card on `/admin/collections`) and look at each document's `ingested_at` timestamp for that window. There is no per-document delete endpoint yet, so removing an individual stray document isn't possible without a code change; if an entire collection only contains unwanted chat uploads, use that collection's "Clear Collection" action to remove all of its documents and chunks at once. This control is gone from chat as of this fix -- see [UI -- File Attachments](ui.md#file-attachments) and [Collections Management UI](#collections-management-ui) above for the current, separated behavior.
