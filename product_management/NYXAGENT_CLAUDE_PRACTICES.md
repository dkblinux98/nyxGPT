...
**Re-verified 2026-09-24.** Every repository figure below was re-checked
against the current tree on that date; figures that moved were updated in
place, and where the movement is itself the point it is flagged. §3b was
...
- **§3b** is Paperclip — added later, after the owner named it — and the
  build-versus-borrow question it forces.
...
- What the CI agents actually run is inline YAML prose: six
  ...
  **572 lines** of `prompt:` between them, and a single **366-line** prompt in
  `claude-code-review.yml` (lines 509–875). Both grew over the month to 2026-09-24;
  the fork is widening, not holding steady.
...
1. Convert each role's charter + runbook + prompt into a **skill** with
   ...
   needs them. Today a developer run reads a 969-line runbook wholesale; a
   review run reads 1,066 lines. That is D-021's problem in its purest form and
...
- `scripts/retrospective/data/reviews_final.json` — **435 review records**,
...
## 3b. Paperclip — the control plane nyxAgent was going to build already exists, MIT-licensed

*Added 2026-09-24, after the owner identified Paperclip
(<https://github.com/paperclipai/paperclip>, docs at <https://docs.paperclip.ing/>)
as "essentially what I'm trying to build with nyxAgent." Read from the project's
own README and documentation on that date, not from secondary coverage.*

### 3b.1 The convergence is real, and closer than the OpenClaw comparison was

`PHASE_7_PLAN.md` already named OpenClaw as nyxAgent's reference architecture.
Paperclip sits one layer **above** it and says so in its own words: *"If OpenClaw
is an employee, Paperclip is the company."* It lists OpenClaw among the
continuous agents it can drive. So the layer `PHASE_7_PLAN.md` proposed building
— the daemon, the heartbeat scheduler, the org model, the dashboard — is the
layer Paperclip already occupies.

Item by item, against decisions this project reached independently:

| Paperclip subsystem | nyxAgent's existing equivalent |
|---|---|
| Atomic task checkout with execution locks — *"no double-work"* | The claim mechanism: `scrummaster_start_issue.sh`, status-before-assign ordering (#3665), WIP-2 and the file-overlap check (`AGENTIC_SDLC_DESIGN.md` §6) |
| First-class blocker dependencies | Native GitHub relationships (D-002), `transitive_blocked_by_issues` |
| Immutable audit log; *"every decision explained"* | `agents/LEDGER.md` (D-005), plus `AGENTS.md`'s auditable-comment-per-state-change rule |
| Persistent agent state — agents *"resume the same task context across heartbeats instead of restarting from scratch"* | §9b, reached from the same diagnosis. **Different answer:** Paperclip persists session state server-side; nyxAgent's rule is that *the repository* carries the memory and the session's own recollection is untrusted input |
| Org chart: roles, reporting lines, permissions, delegation paths | `AGENTS.md` charters and the §5 assignment state machine |
| Goal ancestry — task → project → company goal, so the agent *"sees the 'why,' not just a title"* | Issue → Sprint → Milestone → Release, plus the sprint plan doc (§4) |
| Approval gates; *"nothing ships without your sign-off"* | Owner acceptance, the drain gate, the `For Release` move (D-003) |
| Cost tracking by agent / project / goal / issue / provider / model | `spend.json`, churn cost, artifact-lifetime cost |
| Scheduled heartbeats plus event triggers | GitHub Actions `issues: assigned` today; the `PHASE_7_PLAN.md` daemon tomorrow |
| Adapters — Claude Code, Codex, Cursor, bash, HTTP; *"if it can receive a heartbeat, it's hired"* | **Nothing.** nyxAgent is welded to `claude-code-action` |
| Workspaces, git worktrees, operator branches | **Deliberately not built** — §6 chose scheduling over isolation (see §4.2) |
| Company export/import, multi-tenancy, per-company secrets | **Nothing** — this is precisely the tracker-adapter and per-project-profile seam `NYXAGENT_SEPARATION_PLAN.md` §6 describes as unbuilt |

That is not a loose family resemblance. On the control-plane axis, Paperclip has
independently built most of what the separation plan lists as nyxAgent's
remaining engineering — and it is MIT-licensed, self-hosted, and needs no
account.

### 3b.2 Where it is emphatically *not* what nyxAgent is

Two divergences, both load-bearing.

**It disclaims the entire core.** Paperclip's own "what this is not" list ends
with: *"Not a code review tool — orchestrates work, not pull requests."* The
review loop **is** nyxAgent. The executed-verification gate (D-006), the
reviewable-head gate (D-039), the drain gate, the huddle protocol and its
disagreement-type classification, the three-strike escalation, the
product-vs-process rc-cut decision — none of it has a Paperclip counterpart, and
Paperclip is explicit that it does not intend one.

**The theses are opposites on the human.** Paperclip's stated purpose is
*"zero-human companies"* — *"run a business with zero human intervention."*
`NYXAGENT_SEPARATION_PLAN.md` §2 defines nyxAgent as a team operating *"under a
human owner who supplies judgment, priorities, and acceptance, and who is
treated the way a good team treats its lead: briefed, consulted, never made to
do the typing."* The owner's priority-one capability (§4 of that plan) — voice
huddles, in-meeting note-taking, publication after the room empties — is a
*deepening* of the human's involvement while removing their clerical load.
Paperclip has no surface for that at all.

These are not the same product with different branding. One removes the human;
the other is built around keeping the human's judgment in the loop and taking
away the typing.

### 3b.3 One direct conflict with a standing decision

**P-002 is the mechanism Paperclip built.** Paperclip enforces per-agent monthly
budgets with hard stops: *"When an agent reaches its budget limit, they stop. No
runaway costs."* On 2026-08-09 the owner **rejected** a global hard budget
circuit breaker, and the ledger records it as *"rejected, not pending — do not
re-propose it as a fix for spend incidents"*, on the grounds that a threshold
constant is itself the kind of scripted guard §9 rules out, and that an
intelligent watcher reading spend telemetry gives the same protection with
judgment instead of a constant.

Stated precisely rather than alarmingly: the objection was to a **constant
replacing judgment**, not to a ceiling existing at all. Paperclip also has
warning thresholds and manager alerts, which is the shape §9's watcher wants,
and a configurable hard stop sitting *beneath* an intelligent watcher is a
floor — which §9a's own formula permits. But it would arrive as platform default
behavior rather than as a decision, and adopting a platform whose central
cost-control design is the one this project ruled out should be an explicit
choice, not a side effect.

### 3b.4 The actual question this raises

`PHASE_7_PLAN.md` settled the OpenClaw version of this question: *"build
`nyxagent` purpose-built rather than adopting OpenClaw wholesale — own the whole
stack, consistent with the Phase 8 goal of replacing commercial coding systems
with a system the owner fully controls."* Paperclip re-opens it on better terms,
being both a closer fit and a larger piece of finished work.

The question is **not** "adopt or don't." It is **which layer nyxAgent is**:

- **(a) nyxAgent as a Paperclip company definition.** The SDLC doctrine
  expressed as Paperclip roles, goals, approval policies, adapters and skills,
  running on someone else's control plane. Fastest by a wide margin — the
  separation plan's §7 items 1–2 shrink to configuration. Cost: the control
  plane is not the owner's, and the Phase 8 "fully controls" goal is weakened.
- **(b) Purpose-built, borrowing Paperclip's control-plane design.** The
  `PHASE_7_PLAN.md` decision re-affirmed against a better reference. Paperclip's
  twelve subsystems become the design checklist rather than the dependency —
  most usefully its adapter boundary and its budget/activity model, neither of
  which nyxAgent has in any form.
- **(c) nyxAgent as a Paperclip competitor.** Worth naming so it can be
...
Paperclip's design directly and early: nyxAgent being welded to
`claude-code-action` is a real portability defect for a product whose premise is
reuse across customer projects, and it is the same seam the tracker adapter
already establishes for work tracking.

**What should decide it, and what has not been checked:**

1. **Identity and attribution.** The standing requirement is that agents are the
   actors under their own identities — the constraint `PHASE_7_PLAN.md` calls
   *"unachievable on Actions or a proxied remote session."* Paperclip issues
   per-agent API keys and short-lived run JWTs and traces every mutation to an
   actor, which may satisfy this *better* than Actions does. **Not verified:**
...
§3's plugin proposal and this section are compatible and largely orthogonal.
Under (b), nyxAgent still distributes as a plugin. Under (a) the distribution
question partly dissolves — a Paperclip company definition *is* the
distributable artifact, and Paperclip already ships company export/import with
secret scrubbing.

Either way, **§2.1's skill conversion is the prerequisite**: an agent whose
procedure is 572 lines of inline YAML inside one vendor's GitHub Action cannot
be carried to any other runtime — Paperclip's or nyxAgent's own. That makes
§2.1 the thing to do *before* the topology decision rather than after it, and it
is the strongest argument in this document for that ordering.
...
4. **§2.6 — is Claude Tag evaluated as the SRE agent**, or is the SRE agent
   built as `PHASE_7_PLAN.md` specifies? Identity/attribution is likely the
   deciding constraint.
5. **§5.2 — location.** `agents/knowledge/` assumes the knowledge base is
...
6. **Sequencing against Sprint 9.** Every item here is nyxAgent work and lands
